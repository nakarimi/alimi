...
<template>
<div v-if="canSee" class="vs-sidebar--item" :class="[
...
export default {
  ...
  computed: {
    canSee() {
      ...
      if (this.$router.match(this.to).meta.permission) {
        let user_permissions = localStorage.getItem('user_permissions');
        if(user_permissions){
          if (user_permissions.length > 0 && !this.$router.match(this.to).meta.permission.every(v => user_permissions.includes(v))) {
            return false
          }
        }
      }
