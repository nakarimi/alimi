...
export default {
  ...
  computed: {
    saleTotalCost: function () {
      let x = parseFloat(this.sForm.additional_cost) + parseFloat(this.sForm.service_cost);
  ...
  methods: {
    ...
    submitForm() {
      this.$validator.validateAll('s4Form').then(result => {
        if (result) {
          this.$Progress.start()
          this.sForm.patch(`/api/sale4/${this.$route.params.id}`)
            .then(({
              data
            }) => {
    ...
    // Edit methods
    getSale() {
      this.axios.get(`/api/sale4/${this.$route.params.id}/edit`)
        .then((response) => {
    ...
    setSaleDataEdit(d) {
      if (this.$refs.ekmalat) {
        ...
        this.sForm.client_name = ds.client_name;
        this.sForm.project_id = ds.project;
        this.sForm.destination = ds.destination;
        this.sForm.additional_cost = ds.additional_cost;
        ...
        this.visualFields.additional_cost = ds.additional_cost;
        ...
        if (this.$refs.str) {
          this.$refs.str.getStorages(d.source_id, d.source_type);
