  ...
  <div class="vx-row">
    <vx-card class="height-vh-80">
      ...
      <form data-vv-scope="expensForm">
        ...
        <vs-button :disabled="form.busy" type="filled" @click.prevent="submitData" class="mt-5 block">آپدیت</vs-button>
  ...
  methods: {
    ...
    loadExpense() {
      this.axios.get('/api/expenses/' + this.edit_id)
        .then((response) => {
          ...
          this.form.ammount = response.data.ammount;
          this.visualFields.ammount = this.formatToEnPriceSimple(response.data.ammount);
          this.form.credit_account = response.data.credit_account;
          this.form.debit_account = response.data.debit_account;
    ...
    submitData() {
      this.$validator.validateAll('expensForm').then(result => {
        if (result) {
          this.form.patch(`/api/expenses/${this.edit_id}`)
            .then(() => {
              this.$vs.notify({
